...
while IFS= read -r -d '' git_dir; do
  ...
  if [[ $(git -C "$git_dir" status --porcelain) ]]; then
    repo_url=$(git -C "$git_dir" config --get remote.origin.url)
    branches_with_commits=$(git -C "$git_dir" branch --list --no-color | while read branch; do git -C "$git_dir" log --oneline "$branch" | grep -q . && echo "$branch"; done)
